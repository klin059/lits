...
print(f"Average dice scores for liver and lesion are {np.mean(dice_liver_ls)} and {np.mean(dice_lesion_ls)} consecutively")
    
ID = 0
vol = np.load(os.path.join(param.data_dir, 'vol' + str(ID) + '.npy'))
mask = np.load(os.path.join(param.data_dir, 'mask' + str(ID) + '.npy'))
pred_liver_mask, pred_lesion_mask = lits_util.model_prediction(model, ID, param)
...
for i in range(vol.shape[-1]):
    if np.any(mask[:,:,i]):
        break
lits_util.plot_mask_comparison_over_vol(vol, mask, pred_mask, index_start=i, step = 3)
